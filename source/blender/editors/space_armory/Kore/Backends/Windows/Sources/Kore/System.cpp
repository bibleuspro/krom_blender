...
#define SAFE_RELEASE(x)                                                                                                                                        \
	...
	BOOL CALLBACK enumerateJoysticksCallback(LPCDIDEVICEINSTANCEW ddi, LPVOID context) {
		...
		if (SUCCEEDED(hr)) {
			...
			if (SUCCEEDED(hr)) {
				...
				if (SUCCEEDED(hr)) {
					hr = NULL;//di_pads[padCount]->EnumObjects(enumerateJoystickAxesCallback, nullptr, DIDFT_AXIS);
	...
	void initializeDirectInput() {
		...
		if (SUCCEEDED(hr)) {
			hr = NULL;//di_instance->EnumDevices(DI8DEVCLASS_GAMECTRL, enumerateJoysticksCallback, nullptr, DIEDFL_ATTACHEDONLY);
